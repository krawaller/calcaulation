...
var Home = React.createClass({
    ...
    render: function(){
        return (
            <div>
                <h4>Beskrivning</h4>
                <Mathbox key="desc" id="0" seed={this.state.content[0]} type='TextField' update={this.update.bind(this,0)} />
                <h4>Uträkning</h4>
                {_.map(_.tail(this.state.content),function(txt,n){
                    return (
                        <div key={n+1}>
                            <Mathbox seed={txt} id={n+1} type="MathField" update={this.update.bind(this,n+1)} />
                        </div>
                    );
                }.bind(this))}
                <p>
